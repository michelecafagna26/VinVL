...
class PreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        #     raise ValueError(
        #         "Parameter config in `{}(config)` should be an instance of class `PretrainedConfig`, not {}. "
        #         "To create a model from a pretrained model use "
        #         "`model = {}.from_pretrained(PRETRAINED_MODEL_NAME)`".format(
        #             self.__class__.__name__, config.__class__.__name__, self.__class__.__name__
        #         ))
