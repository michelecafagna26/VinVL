...
class OscarTensorizer(object):
    def __init__(self, tokenizer, device="cpu", max_img_seq_length=50, max_seq_length=70,
                 ...
                 is_train=False, input_params=None):
    ...
    def decode(self, outputs):
        """
        Decode Oscar outputs

        Parameters
        ----------
        outputs: List[torch.Tensor]  with a torch.Tensor of shape (batch_size * num_keep_best * max_len)

        Returns
        -------
        output_dict: dict-like { id: [ "caption": str, "conf": float32 ] }
    ...
    def encode(self, img_feat, caption="", labels=None,
        ...
        """
        Encode Oscar inputs into Tensors
        Parameters
        ----------
        img_feat: torch.Tensor with shape (batch_size, num_feat, feat_size)
        caption: str
        labels: List[str]
        cls_token_segment_id: int, default: 0
        pad_token_segment_id: int, default: 0
        sequence_a_segment_id: int, default: 0
        sequence_b_segment_id: int, default: 1
        use_cbs: bool, default: False

        Returns
        -------
        inputs_params: dict-like {'input_ids': torch.Tensor, 'attention_mask': torch.Tensor,
